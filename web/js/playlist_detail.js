...
const element = document.querySelector("[data-fetch-page]");
const fetchPage = element.getAttribute("data-fetch-page");


let tymSongs = new Map();
...
const app = {
    ...
    setTymSong: function (){
        this.songs.forEach(song => {
            tymSongs.set(song.song_id, song.liked);
        })
    },
    ...
    handleEvents: function () {
        ...
        tymBtn.onclick = () => {
            ...
            if (tymSongs.get(this.currentSong.song_id) == false) {
                tymBtn.classList.add("active");
                this.addTym(this.currentSong.song_id);
                 tymSongs.set(this.currentSong.song_id, true)
            }else{
                tymBtn.classList.remove("active");
                this.deleteTym(this.currentSong.song_id);
                tymSongs.set(this.currentSong.song_id, false)
            }
        };
        ...
        playlist.onclick = (e) => {
            ...
            if (songNode && !optionNode) {
                ...
                console.log(123); // Xử lý option ở đây
            }
        };
    ...
    addTym: function (songId) {
        fetch("LikeServlet", {
            method: "POST",
            headers: {"Content-Type": "application/x-www-form-urlencoded"},
            body: `action=add&songId=${songId}`
        })
    ...
    deleteTym: function (songId) {
        fetch("LikeServlet", {
            method: "POST",
            headers: {"Content-Type": "application/x-www-form-urlencoded"},
            body: `action=delete&songId=${songId}`
        })
                .then(response => response.text())
                .then(data => {
                    console.log(data);
    ...
    loadCurrentSong: function () {
        ...
        if (tymSongs.get(this.currentSong.song_id) == true) {
            tymBtn.classList.add("active");
        } else {
            tymBtn.classList.remove("active");
        }
        ...
        console.log(tymSongs.get(this.currentSong.song_id));
    },
